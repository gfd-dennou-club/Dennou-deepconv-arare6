...
module ChemCalc
  ...
  use ChemData,   only: ChemData_init,                 & !初期化
    ...
    &                   ChemData_CpRef,                & !標準状態での単位質量当たりの比熱
    &                   ChemData_CpPerMolRef,          & !標準状態での単位モル当たりの比熱
    &                   ChemData_CvRef,                & !標準状態での単位質量当たりの比熱
    &                   ChemData_MolWt,                & !分子量
    &                   ChemData_GasR                    !気体定数 [J/K kg]
  ...
  integer,  save :: a_kmin(ChemData_SpcNum)   !物質毎に決めた配列の下限
  integer,  save :: a_kmax(ChemData_SpcNum)   !物質毎に決めた配列の下限
  real(DP), save :: a_SwAmp(ChemData_SpcNum)  !スイッチ. AMP を使う場合は 1.0, そうでなければ 0.0  
  real(DP), save :: a_SwAnt(ChemData_SpcNum)  !スイッチ. Antoine を使う場合は 1.0, そうでなければ 0.0
  real(DP), save :: a_antA(ChemData_SpcNum)   !Antoine の蒸気圧式の A 係数
  real(DP), save :: a_antB(ChemData_SpcNum)   !Antoine の蒸気圧式の B 係数
  real(DP), save :: a_antC(ChemData_SpcNum)   !Antoine の蒸気圧式の C 係数
  real(DP), save :: a_antU(ChemData_SpcNum)   !Antoine の蒸気圧式の単位換算のための係数
  real(DP), save :: a_ampA(ChemData_SpcNum)   !AMP 式の蒸気圧式の A 係数
  real(DP), save :: a_ampB(ChemData_SpcNum)   !AMP 式の蒸気圧式の B 係数
  real(DP), save :: a_ampC(ChemData_SpcNum)   !AMP 式の蒸気圧式の C 係数
  real(DP), save :: a_ampD(ChemData_SpcNum)   !AMP 式の蒸気圧式の D 係数
  real(DP), save :: a_ampE(ChemData_SpcNum)   !AMP 式の蒸気圧式の E 係数
  real(DP), save :: a_MolWt(ChemData_SpcNum)  !分子量
  
  ! 公開するサブルーチンに public 属性を付ける
  !
  public ChemCalc_Init                            !初期化ルーチン
  public ChemCalc_Init2                           !初期化ルーチン
  public MolWt                                    !分子量
  public GasR                                     !気体定数
...
!!    ! 物質毎に違うことは考えていないので, 物質に対するループは回さない. 
!!    !
      ...
      do j = 1, ny
        do i = 1, nx
          ...
          LogSvapPress =                                           &
            ...
            &       + a_ampE(ID) * ( xyz_temp(i,j,k) ** 2.0d0 )    &
            &       + dlog(1.0d-1)                                 &
            &      ) * a_SwAmp(ID)                                 &
            &    + (                                               &
            &       + (                                            &
