...
!!!  
  ...
  !
  real(DP), parameter :: nu = 1.0d0      ! 粘性係数
...
!!!
  do while ( Nstep <= NstepLong )
     ...
     ! tendency (傾き) の計算
     ! 拡散方程式は安定性のために 1 つ前の値 (末尾 Bl) で傾きを計算する. 
     !
     xyz_DZetaDtNl =                                     &
          &  nu * (                                      &
          &           xyz_dx_pyz(pyz_dx_xyz(xyz_ZetaBl)) &
          &         + xyz_dz_xyr(xyr_dz_xyz(xyz_ZetaBl)) &
          &       )  
     !
     ! 温位の積分 (leap-frog)
     !
...
!!!  
  ...
  subroutine fileset_init
    ...
    call HistoryCreate( &                                  ! ヒストリー作成
      file='diffuse.nc', title='2D diffusion model',                             &
